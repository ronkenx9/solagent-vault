...
export class RuleEngine {
  ...
  private localPolicies: Map<string, AgentPolicy> = new Map();
  private localTxLog: Map<string, number[]> = new Map();
  ...
  async registerPolicy(policy: AgentPolicy): Promise<void> {
    if (this.supabase) {
      ...
      if (programsToInsert.length > 0) {
        ...
        if (progError) console.error(`[RuleEngine] Failed to register programs for ${policy.agentId}:`, progError);
      }
    } else {
  ...
  async updatePolicy(agentId: string, updates: Partial<Omit<AgentPolicy, 'agentId'>>): Promise<boolean> {
    if (this.supabase) {
      ...
      if (updates.maxLamportsPerTx !== undefined) dbUpdates.max_lamports_per_tx = updates.maxLamportsPerTx;
      if (updates.maxTxPerMinute !== undefined) dbUpdates.max_tx_per_minute = updates.maxTxPerMinute;
  ...
  async getPolicy(agentId: string): Promise<AgentPolicy | undefined> {
    if (this.supabase) {
      ...
      if (error || !agent) return undefined;
      ...
      return {
        ...
        paused: agent.paused,
        allowedPrograms: agent.agent_allowed_programs.map((p: any) => p.program_id)
      };
    } else {
   ...
   */
  async check(intent: Intent): Promise<RuleCheckResult> {
   ...
   */
  async logTransaction(intent: Intent, status: 'EXECUTED' | 'BLOCKED' | 'FAILED', signature: string | null = null, checkResult?: RuleCheckResult) {
    if (this.supabase) {
